...
describe('Testing getJobsByCategory route', () => {
  ...
  it('In the success path, it should return 200 status code and the success message.', (done) => {
    supertest(app)
      ...
      .end((err, res) => {
        if (err) return done(err);
        expect(res.body.msg).toBe('The jobs of the category you searched for returned successfully!');
        return done();
      });
  });
  ...
  it('In the failure path, it should return 404 status code and not found msg.', (done) => {
    supertest(app)
      ...
      .end((err, res) => {
        if (err) return done(err);
        expect(res.body.msg).toBe('The category you searched for does not exist!');
        return done();
      });
  });
});
