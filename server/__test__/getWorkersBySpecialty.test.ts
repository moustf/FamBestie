...
describe('Testing get workers by specialty route for admin role', () => {
  it('In the success path, it should return 200 status code.', (done) => {
    supertest(app)
      .get('/api/v1/admin/worker?specialty=driver')
      .set('Cookie', [
  ...
  it('In the success path, it should return 200 status code and returned successfully message.', (done) => {
    supertest(app)
      ...
      .end((err, res) => {
        if (err) return done(err);
        expect(res.body.msg).toBe('The workers who share the specialty did return successfully!');
        return done();
      });
  });
